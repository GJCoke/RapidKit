...
更改 [执行策略](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_execution_policies?view=powershell-7.4#powershell-execution-policies)
允许运行来自互联网的脚本。
:::
...
```bash
git clone https://github.com/GJCoke/rapidkit.git
```
...
::: info 说明
Celery 的 Broker 和 Backend 均使用 Redis，连接地址由 `CELERY_REDIS_URL` 属性自动生成，格式为
`redis://:password@host:port/database`。其中 `CELERY_REDIS_DATABASE` 默认为 `1`，与主应用 Redis 数据库 (默认 `0`) 隔离。
:::
...
如果启动 Worker 后在前端看不到 Worker 状态信息，请确认 `ENABLE_CELERY_MONITOR=true` 已在 `.env` 中设置。该选项控制是否加载事件消费和
Worker 监控模块。
