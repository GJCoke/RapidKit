...
项目的 HTTP 请求基于 `@rapidkit/axios` 内部包封装，该包位于 `packages/axios/`。前端应用在 `src/service/` 目录下配置拦截器并组织
API 模块。
...
::: tip
建议在 `src/typings/` 下创建全局类型声明文件，将常用的 API 类型集中管理，方便在业务代码中直接使用 `Api.Auth.LoginBody`
等简短引用。
:::
