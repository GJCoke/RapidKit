...
from src.domains.system.crud import ActivityLogCRUD
    ...
    async def log_activity(
        ...
        *,
        ...
        if self.sio is not None:
            try:
                from src.domains.system.schemas import ActivityResponse

                payload = ActivityResponse.model_validate(record).serializable_dict()
                await self.sio.emit("dashboard:activity", payload, namespace="/dashboard")  # ty: ignore[unresolved-attribute]
            except Exception:
        ...
        async def _do() -> None:
            try:
                ...
                async with AsyncSessionLocal() as session:
                    service = ActivityService(session=session, sio=socket)
