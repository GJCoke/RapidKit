...
def get_registered_tasks(celery_app: object) -> list[str]:
    celery_app.loader.import_default_modules()
    return sorted([name for name in celery_app.tasks if not name.startswith("celery.")])


async def trigger_task(celery_app: object, task_name: str, args: list, kwargs: dict) -> TriggerTaskResponse:
...
def ping_worker(celery_app: object, hostname: str) -> bool:
    try:
...
def shutdown_worker(celery_app: object, hostname: str) -> None:
    try:
...
def pool_grow(celery_app: object, hostname: str, n: int = 1) -> None:
    try:
...
def pool_shrink(celery_app: object, hostname: str, n: int = 1) -> None:
    try:
...
def _check_pool_reply(reply: list | None, hostname: str, action: str) -> None:
    if not reply:
        logger.warning("No reply from worker %s for %s", hostname, action)
        raise AppException(StatusCode.WORKER_CONTROL_FAILED)
...
def add_consumer(celery_app: object, hostname: str, queue: str) -> None:
    try:
...
def cancel_consumer(celery_app: object, hostname: str, queue: str) -> None:
    try:
...
def get_active_tasks(celery_app: object, hostname: str) -> list[dict]:
    try:
...
def get_reserved_tasks(celery_app: object, hostname: str) -> list[dict]:
    try:
        inspector = celery_app.control.inspect(destination=[hostname], timeout=CONTROL_TIMEOUT)
...
def revoke_task(celery_app: object, task_id: str, *, terminate: bool = False) -> None:
    try:
