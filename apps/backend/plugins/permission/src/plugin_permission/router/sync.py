...
from plugin_permission.router.crud import RouterCRUD
...
async def sync_routes_on_startup(app: FastAPI) -> None:
    """插件 on_startup 回调：将应用路由同步到数据库。"""
...
def diff_api_routes(
    ...
    def route_key(r: Any) -> str:
        return build_route_key(r.methods or [], r.path)
