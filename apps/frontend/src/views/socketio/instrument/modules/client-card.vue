<script setup lang="ts">
  ...
  defineProps<Props>()
  const emit = defineEmits(["kick", "click"])
</script>
    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex gap-2 items-center shrink-0">
          ...
          <n-button v-if="showAction" circle size="tiny" type="error" quaternary @click.stop="emit('kick', client.id)">
      ...
      <div class="grid grid-cols-2 gap-4 bg-zinc-100/50 dark:bg-zinc-800/50 p-2 rounded-md">
        <div class="flex flex-col">
          <span class="text-[9px] text-zinc-400 uppercase">Namespace</span>
          <span class="text-[11px] font-medium truncate">{{ client.nsp }}</span>
        </div>
        <div class="flex flex-col">
          <span class="text-[9px] text-zinc-400 uppercase">Address</span>
