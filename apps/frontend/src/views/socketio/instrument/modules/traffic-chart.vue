<script setup lang="ts">
  ...
  const { domRef, updateOptions } = useEcharts(() => ({
    ...
    series: [
      {
        name: $t("page.socketio.instrument.received"),
        type: "bar",
        data: [] as number[],
        itemStyle: { color: "#18a058" },
      },
      { name: $t("page.socketio.instrument.sent"), type: "bar", data: [] as number[], itemStyle: { color: "#f0a020" } },
    ],
  }))
  ...
  watch(
    ...
    (newEvents) => {
      ...
      updateOptions((opts) => {
        opts.xAxis.data = chartData.value.time
        opts.series[0].data = chartData.value.received
        opts.series[1].data = chartData.value.sent
        return opts
      })
    },
    { deep: true },
