<script setup lang="ts">
  ...
  async function handleRun() {
    ...
    execStatus.value = data.exit_code === 0 ? "success" : "error"
  }
...
<template>
  <div class="h-full flex-col-stretch overflow-hidden">
    <!-- Toolbar -->
    <div class="flex items-center gap-12px px-16px py-8px b-b b-b-solid b-b-[var(--n-border-color)]">
      ...
      <template v-if="scriptData">
        <span class="font-bold text-16px">{{ scriptData.name }}</span>
        <NTag size="small" :bordered="false">{{ scriptData.language }}</NTag>
      </template>
    ...
    <!-- Output Panel -->
    <OutputPanel :output="execOutput" :error="execError" :status="execStatus" :runtime="execRuntime" />
  </div>
</template>
