...
```ts
import { Crypto, jsonClone, nanoid, createStorage, createLocalforage } from "@monorepo-example/utils"
```

## 构建方式

通过 `@monorepo-example/builder`(Rollup)构建,输出 ESM / CJS / IIFE 三种格式,入口为 `dist/index.esm.js`。
...
基于 `klona/json`,仅克隆 JSON 安全的值(不含函数、Symbol 等),性能优于 `structuredClone` 和 `JSON.parse(JSON.stringify(...))`。
...
::: warning
`jsonClone` 不支持 `Date`、`RegExp`、`Map`、`Set` 等非 JSON 类型,这些值会被丢弃或转换。如需完整结构化克隆,请使用浏览器原生 `structuredClone`。
:::
