...
```ts
import { createRequest, createFlatRequest, BACKEND_ERROR_CODE, REQUEST_ID_KEY } from "@monorepo-example/axios"
```
...
| `qs`                      | Query 参数序列化          |
| `@monorepo-example/utils` | 使用 `nanoid` 生成请求 ID |
...
```ts
import { createFlatRequest, BACKEND_ERROR_CODE } from "@monorepo-example/axios"
