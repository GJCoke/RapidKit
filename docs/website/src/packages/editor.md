...
import type { MonacoEditorProps, OutputPanelProps, OutputStatus } from "@monorepo-example/editor"
```
...
<script setup lang="ts">
  import { ref } from "vue"
  import { MonacoEditor } from "@monorepo-example/editor"
...
::: tip
`MonacoEditor` 和 `OutputPanel` 可组合使用,搭建完整的在线代码编辑与执行界面。`MonacoEditor` 的 `run` 事件可直接触发代码提交,结果展示在 `OutputPanel` 中。
:::
