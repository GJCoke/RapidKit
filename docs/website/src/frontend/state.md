...
项目使用 Pinia 3 进行状态管理，全部采用 Setup Store（Composition API）语法定义。Store 文件位于 `src/store/modules/` 目录下，每个模块一个独立文件夹。
...
颜色变更时，通过 `@monorepo-example/color` 包的 `getPaletteColorByNumber` 生成推荐色板。
