...
颜色变更后，`@monorepo-example/color` 包的 `getPaletteColorByNumber` 会生成推荐色板，并通过 `setupThemeVarsToGlobal()` 注入 CSS 变量。
...
项目在 Naive UI 基础上封装了一些业务组件，位于 `src/components/` 目录下。这些组件同样通过 `unplugin-vue-components` 自动导入，无需手动 import。
