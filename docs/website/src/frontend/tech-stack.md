...
前端应用 `@monorepo-example/frontend` 依赖以下 Monorepo 内部包：

```
@monorepo-example/frontend
  ├── @monorepo-example/axios    → HTTP 请求封装（依赖 @monorepo-example/utils）
  ├── @monorepo-example/hooks    → 通用 Composition Hooks（依赖 @monorepo-example/axios）
  ├── @monorepo-example/color    → 主题色彩工具（依赖 @monorepo-example/utils）
  ├── @monorepo-example/utils    → 基础工具函数
  └── @monorepo-example/editor   → Monaco Code Editor 组件（独立包）
```
