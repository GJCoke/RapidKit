...
本项目使用多种静态分析工具保证代码质量：前端通过 **TypeScript** 类型检查，后端通过 **mypy** 和 **ty** 双重类型检查，全项目通过 **cSpell** 拼写检查。
...
项目通过 Turborepo 调度各子项目的类型检查任务，执行 `tsc --noEmit`（仅检查类型，不输出编译文件）。Turborepo 的缓存机制会跳过无代码变更的子项目，提升 CI/CD 效率。
...
::: info 说明
pre-push Hook 会自动执行 `pnpm typecheck`，阻止类型错误的代码推送到远程仓库。
:::

## mypy（Python 类型检查）

[mypy](https://mypy.readthedocs.io/) 是 Python 的静态类型检查工具，配置位于 `apps/backend/pyproject.toml` 的 `[tool.mypy]` 段。
...
[ty](https://github.com/astral-sh/ty) 是 Astral 团队（Ruff 作者）开发的下一代 Python 类型检查器，使用 Rust 编写，速度极快。配置位于根目录 `pyproject.toml` 的 `[tool.ty]` 段。
...
| 工具       | 命令                              | 作用                             |
| ---------- | --------------------------------- | -------------------------------- |
| TypeScript | `pnpm typecheck`                  | 全项目类型检查（Turborepo 并行） |
| mypy       | `uv run mypy src`                 | Python 类型检查                  |
| mypy       | `pnpm --filter backend lint:lint` | Monorepo 统一命令                |
| ty         | `uv run ty check src`             | 下一代 Python 类型检查           |
| ty         | `pnpm --filter backend typecheck` | Monorepo 统一命令                |
| cSpell     | `pnpm lint:spellcheck`            | 拼写检查                         |

::: warning 注意
提交代码前建议依次运行 `lint:format`、`lint:lint` 和 `typecheck`，确保代码风格和类型安全性。实际上 Husky Hooks 已自动覆盖这些检查，但手动运行有助于提前发现问题。
:::
