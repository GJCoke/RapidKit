...
Vue 配置使用 `vue-eslint-parser` 解析 SFC 模板，内部 `<script>` 由 TypeScript 解析器处理。组件命名规则允许 `index.vue`、`App.vue` 等单词文件名。
...
[Ruff](https://docs.astral.sh/ruff/) 是用 Rust 编写的高性能 Python Linter + Formatter，替代 flake8 + isort + black。配置位于根目录 `pyproject.toml` 的 `[tool.ruff]` 段。
