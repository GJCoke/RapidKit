...
from rapidkit_framework.events import Event, event_bus
...
from rapidkit_framework.plugin_config import load_plugin_config
...
def validate_requires(manifests: list[PluginManifest]) -> None:
    """
    Validate that all 'requires' protocols have a corresponding 'provides'.

    Raises RuntimeError if any required protocol is not provided by any loaded plugin.
    """
    all_provides: set[type] = set()
    for m in manifests:
        all_provides.update(m.provides)

    for m in manifests:
        for protocol in m.requires:
            if protocol not in all_provides:
                raise RuntimeError(f"Plugin '{m.name}' requires {protocol.__name__}, but no loaded plugin provides it.")


def resolve_services(registry: ServiceRegistry, manifests: list[PluginManifest]) -> None:
    """
    Call each plugin's service_factories in topological order to register implementations.

    Args:
        registry: The ServiceRegistry to populate.
        manifests: Topologically sorted plugin list.
    """
    for manifest in manifests:
        for protocol, factory in manifest.service_factories.items():
            factory(registry)
            logger.debug(
                "Plugin '{}' registered service for {}.",
                manifest.name,
                protocol.__name__,
            )
